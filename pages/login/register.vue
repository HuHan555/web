<template>
  <view class="content">
    ...
    <view class="list">
	  ...
	  <view class="list-call">
	    <image class="img" src="/static/shilu-login/2.png"></image>
	    <input class="sl-input" v-model="repassword" type="text" maxlength="32" placeholder="确认登录密码" :password="!reshowPassword" />
	    <image class="img" :src="reshowPassword?'/static/shilu-login/op.png':'/static/shilu-login/cl.png'" @tap="redisplay"></image>
	  </view>
      <!-- <view class="list-call">
      ...
      <view class="list-call">
        <image class="img" src="/static/shilu-login/3.png"></image>
        <input class="sl-input" v-model="email" type="text" placeholder="邮箱" />
      </view>
...
<script>
    ...
    methods: {
      ...
      bindLogin() {
		...
		if(!myfun.checkEmail(this.email)){
			uni.showToast({title: '邮箱格式错误',icon: 'none'});
			return;
		}
		
        uni.request({
          url: this.ip+'doRegist',
          data: {
            phone: this.phone,
